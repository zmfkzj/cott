...
def _medical_subtotal(lines: CottList[Quantity]) -> int:
    subtotal: int = 0
    for line in lines:
...
def _grocery_subtotal(lines: CottList[Quantity]) -> int:
    subtotal: int = 0
    for line in lines:
...
def _drinks_subtotal(lines: CottList[Quantity]) -> int:
    subtotal: int = 0
    for line in lines:
...
def _rounded_percent(cents: int, percent: int) -> int:
    quotient: int
    remainder: int
...
def calculate_bill(medical: CottList[Quantity], grocery: CottList[Quantity], drinks: CottList[Quantity]) -> Result[BillTotals, BillingError]:
    ...
    medical_cents: int = _medical_subtotal(medical)
    grocery_cents: int = _grocery_subtotal(grocery)
    drinks_cents: int = _drinks_subtotal(drinks)
    medical_tax_cents: int = _rounded_percent(medical_cents, 5)
    grocery_tax_cents: int = _rounded_percent(grocery_cents, 5)
    drinks_tax_cents: int = _rounded_percent(drinks_cents, 10)
