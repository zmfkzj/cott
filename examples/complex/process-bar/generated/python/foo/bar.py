...
def validate_payload(data: InputPayload) -> Result[InputPayload, BarError]:
    ...
    if _expected_error is None and ((len((data).data) == 0)):
        ...
        _expected_error_span = {"end_byte":1047,"end_column":58,"end_line":44,"start_byte":994,"start_column":5,"start_line":44}
    ...
    try:
        ...
        if _error.span is None:
            _error.span = {"end_byte":1065,"end_column":1,"end_line":48,"start_byte":615,"start_column":1,"start_line":35}
        raise
    except SystemExit as _error:
        raise CottContractViolation("implementation raised SystemExit", symbol="foo.bar.validate_payload", phase="implementation-call", span={"end_byte":1065,"end_column":1,"end_line":48,"start_byte":615,"start_column":1,"start_line":35}, expected="ordinary return or declared Never process.exit", actual="SystemExit") from _error
    except Exception as _error:
        raise CottContractViolation("implementation raised an undeclared exception", symbol="foo.bar.validate_payload", phase="implementation-call", span={"end_byte":1065,"end_column":1,"end_line":48,"start_byte":615,"start_column":1,"start_line":35}, expected="declared Result error or ordinary return", actual=type(_error).__name__) from _error
    ...
    if type(_result) is Err:
        if _expected_error is not None:
            ...
            raise CottContractViolation("returned error is not allowed", symbol="foo.bar.validate_payload", phase="error", span={"end_byte":1065,"end_column":1,"end_line":48,"start_byte":615,"start_column":1,"start_line":35}, expected="declared unconditional error variant", actual=type(_result.error).__name__)
    ...
    if type(_result) is Ok and True:
        validated = _result.value
        if not (((validated).data == (data).data)):
            raise CottContractViolation("ensures clause failed", symbol="foo.bar.validate_payload", clause="ensures:1", phase="ensures", span={"end_byte":838,"end_column":64,"end_line":40,"start_byte":779,"start_column":5,"start_line":40}, expected="true", actual="false")
    if type(_result) is Ok and True:
        validated = _result.value
        if not ((((validated).declared_size).value == ((data).declared_size).value)):
            raise CottContractViolation("ensures clause failed", symbol="foo.bar.validate_payload", clause="ensures:2", phase="ensures", span={"end_byte":920,"end_column":82,"end_line":41,"start_byte":843,"start_column":5,"start_line":41}, expected="true", actual="false")
    if type(_result) is Ok and True:
        validated = _result.value
        if not (((validated).format == (data).format)):
...
def process_payload_bytes(data: bytes, options: BarOptions) -> Result[bytes, BarError]:
    ...
    try:
        ...
        if _error.span is None:
            _error.span = {"end_byte":1405,"end_column":1,"end_line":60,"start_byte":1065,"start_column":1,"start_line":48}
        raise
    except SystemExit as _error:
        raise CottContractViolation("implementation raised SystemExit", symbol="foo.bar.process_payload_bytes", phase="implementation-call", span={"end_byte":1405,"end_column":1,"end_line":60,"start_byte":1065,"start_column":1,"start_line":48}, expected="ordinary return or declared Never process.exit", actual="SystemExit") from _error
    except Exception as _error:
        raise CottContractViolation("implementation raised an undeclared exception", symbol="foo.bar.process_payload_bytes", phase="implementation-call", span={"end_byte":1405,"end_column":1,"end_line":60,"start_byte":1065,"start_column":1,"start_line":48}, expected="declared Result error or ordinary return", actual=type(_error).__name__) from _error
    ...
    if type(_result) is Err:
        if _expected_error is not None:
            ...
            raise CottContractViolation("returned error is not allowed", symbol="foo.bar.process_payload_bytes", phase="error", span={"end_byte":1405,"end_column":1,"end_line":60,"start_byte":1065,"start_column":1,"start_line":48}, expected="declared unconditional error variant", actual=type(_result.error).__name__)
    elif _expected_error is not None:
...
def build_output(data: bytes, source_size: PayloadSize, format: PayloadFormat) -> OutputPayload:
    ...
    try:
        ...
        if _error.span is None:
            _error.span = {"end_byte":1730,"end_column":1,"end_line":71,"start_byte":1405,"start_column":1,"start_line":60}
        raise
    except SystemExit as _error:
        raise CottContractViolation("implementation raised SystemExit", symbol="foo.bar.build_output", phase="implementation-call", span={"end_byte":1730,"end_column":1,"end_line":71,"start_byte":1405,"start_column":1,"start_line":60}, expected="ordinary return or declared Never process.exit", actual="SystemExit") from _error
    except Exception as _error:
        raise CottContractViolation("implementation raised an undeclared exception", symbol="foo.bar.build_output", phase="implementation-call", span={"end_byte":1730,"end_column":1,"end_line":71,"start_byte":1405,"start_column":1,"start_line":60}, expected="declared Result error or ordinary return", actual=type(_error).__name__) from _error
    _result = _cott_validate_abi(_result, OutputPayload, path="$.return")
    if not (((_result).data == data)):
        raise CottContractViolation("ensures clause failed", symbol="foo.bar.build_output", clause="ensures:1", phase="ensures", span={"end_byte":1630,"end_column":32,"end_line":65,"start_byte":1603,"start_column":5,"start_line":65}, expected="true", actual="false")
...
def process_bar(data: InputPayload, options: BarOptions) -> Result[OutputPayload, BarError]:
    ...
    if _expected_error is None and ((len((data).data) == 0)):
        _expected_error = BarError_InvalidPayload
        _expected_error_span = {"end_byte":2173,"end_column":58,"end_line":80,"start_byte":2120,"start_column":5,"start_line":80}
        _expected_error_clause = "error:4"
    try:
        _implementation = _cott_load("_cott_impl/foo/bar/process_bar.py", "724e814cc557a0256118711ce2be493ffe68e9d4e8983dfbe5f8715d4ecce028", "process_bar", expected_project_name="process-bar", expected_cott_symbol="foo.bar.process_bar")
        _result = _implementation(data, options)
    except CottContractViolation as _error:
        if _error.symbol is None or _error.symbol == "_cott_load":
            _error.symbol = "foo.bar.process_bar"
        if _error.span is None:
            _error.span = {"end_byte":2271,"end_column":1,"end_line":85,"start_byte":1730,"start_column":1,"start_line":71}
        raise
    except SystemExit as _error:
        raise CottContractViolation("implementation raised SystemExit", symbol="foo.bar.process_bar", phase="implementation-call", span={"end_byte":2271,"end_column":1,"end_line":85,"start_byte":1730,"start_column":1,"start_line":71}, expected="ordinary return or declared Never process.exit", actual="SystemExit") from _error
    except Exception as _error:
        raise CottContractViolation("implementation raised an undeclared exception", symbol="foo.bar.process_bar", phase="implementation-call", span={"end_byte":2271,"end_column":1,"end_line":85,"start_byte":1730,"start_column":1,"start_line":71}, expected="declared Result error or ordinary return", actual=type(_error).__name__) from _error
    ...
    if type(_result) is Err:
        if _expected_error is not None:
            ...
            raise CottContractViolation("returned error is not allowed", symbol="foo.bar.process_bar", phase="error", span={"end_byte":2271,"end_column":1,"end_line":85,"start_byte":1730,"start_column":1,"start_line":71}, expected="declared unconditional error variant", actual=type(_result.error).__name__)
    elif _expected_error is not None:
    ...
    if type(_result) is Ok and True:
        output = _result.value
        if not (((output).data == (data).data)):
            raise CottContractViolation("ensures clause failed", symbol="foo.bar.process_bar", clause="ensures:1", phase="ensures", span={"end_byte":1978,"end_column":58,"end_line":76,"start_byte":1925,"start_column":5,"start_line":76}, expected="true", actual="false")
    if type(_result) is Ok and True:
        output = _result.value
        if not ((((output).source_size).value == ((data).declared_size).value)):
            raise CottContractViolation("ensures clause failed", symbol="foo.bar.process_bar", clause="ensures:2", phase="ensures", span={"end_byte":2052,"end_column":74,"end_line":77,"start_byte":1983,"start_column":5,"start_line":77}, expected="true", actual="false")
    if type(_result) is Ok and True:
        output = _result.value
        if not (((output).format == (data).format)):
