...
def _valid_ip_block(block: str) -> bool:
    ...
    if ":" in address:
        if not _valid_ipv6(address):
            return False
        return not sep or _parse_decimal(prefix, 128)
    if not _valid_ipv4(address):
        return False
    return not sep or _parse_decimal(prefix, 32)
...
def select_geo_route(policy: NetworkPolicy) -> Result[NetworkPolicy, MediaError]:
    match policy.geo_mode:
        ...
        case GeoBypassMode_Country():
            if not _valid_country(policy.geo_country):
                return Err(error=MediaError_GeoRestricted(message="invalid geo-bypass country code"))
            return Ok(value=policy)
