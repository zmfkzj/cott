...
from real.toolong_types import (
    ...
    LogSource,
    ToolongError,
    ToolongError_CompressedAppendUnsupported,
...
def _json_object_timestamp(text: str) -> str | None:
...
def _access_timestamp(text: str) -> str | None:
...
def _iso_timestamp(text: str) -> str | None:
...
def _classify(text: str) -> tuple[EntryKind, Option[str]]:
    if _valid_json(text):
        json_timestamp = _json_object_timestamp(text)
        if json_timestamp is None:
            return (EntryKind_Json(), Nothing())
        return (EntryKind_Json(), Some(value=json_timestamp))
    access_timestamp = _access_timestamp(text)
    if access_timestamp is not None:
        return (EntryKind_Access(), Some(value=access_timestamp))
    timestamp = _iso_timestamp(text)
    if timestamp is None:
        option_timestamp: Option[str] = Nothing()
    else:
        option_timestamp = Some(value=timestamp)
    folded = text.casefold()
    if "error" in folded or "exception" in folded or "fatal" in folded or "traceback" in folded:
        return (EntryKind_Error(), option_timestamp)
    return (EntryKind_Plain(), option_timestamp)
...
def read_appended(source: LogSource, from_byte: U64, limit: U64) -> Result[LogPage, ToolongError]:
    ...
    if suffix == ".bz" or suffix == ".bz2":
        return Err(error=ToolongError_CompressedAppendUnsupported(path=source.path))
    ...
    if from_byte > size:
        return Err(error=ToolongError_InvalidOffset(path=source.path, offset=from_byte))
    entries: list[LogEntry] = []
    byte_offset = from_byte
    record = 1
    with source.path.open("rb") as stream:
        stream.seek(from_byte)
        while len(entries) < limit:
            raw = stream.readline()
            if raw == b"":
                break
            record_data = _record_bytes(raw)
            if not _valid_utf8(record_data):
