...
use super::verify;
use super::{KotlinBinding, KotlinCallable, KotlinOwner};
...
pub(crate) fn generate(
    ...
    if symbol.is_none() {
        repairable.extend(
            project
                .bindings
                .iter()
                .filter(|binding| binding.owner == KotlinOwner::Agent)
                .map(|binding| binding.cott_symbol.clone()),
        );
    }

    // Freeze prompts in canonical source order before accepting any wave result.
    // A full invocation also freezes existing agent bindings because whole-target
    // validation may identify one of them as the binding that needs repair.
        ...
        .filter(|callable| repairable.contains(&callable.symbol))
    {
        let resolved_source = project
            ...
            .iter()
            .find(|binding| {
                binding.owner == KotlinOwner::Agent && binding.cott_symbol == callable.symbol
            })
        ...
        let initial = match prepare_prompt(
            ...
            Err(message) => {
                eprintln!(
                    "error: prepare Kotlin generation prompt for `{}`: {message}",
                    callable.symbol
                );
                return 4;
            }
    ...
    let selected_work = work
        ...
        .collect::<Vec<_>>();
    if agent.is_none() {
        eprintln!("error: unresolved selected Kotlin callable requires `--agent codex|claude|omp`");
        return 2;
    }

    let mut candidates = BTreeMap::<String, KotlinBinding>::new();
    let mut runs = BTreeMap::<String, AgentRun>::new();
    let mut pending = unresolved.clone();
    let mut generation_failed = false;
    let kind = agent.expect("selected unresolved Kotlin work requires an agent");
            ...
            match validate_complete(&project, &bindings) {
                ...
                Err(message) => {
                    ...
                    if attempts == VERIFICATION_RETRIES || repair.is_empty() {
                        eprintln!("error: generated Kotlin candidate validation failed: {message}");
                        verification_failed = true;
                        break;
                    }
                    attempts += 1;
                    eprintln!(
                        "Kotlin complete-candidate validation retry {attempts}/{VERIFICATION_RETRIES}: {message}"
                    );
                    let feedback = format!(
                        "Complete Kotlin emitter/compiler/runtime validation failed. Repair the selected implementation without weakening the canonical contract. Exact diagnostic:\n{message}"
                    );
                    let outcome = run_generation_waves(
                        &repair,
                        jobs,
...
fn run_generation_waves(
    ...
    rules: Option<&str>,
    feedback: Option<&str>,
    candidates: &mut BTreeMap<String, KotlinBinding>,
    ...
    for attempt in 0..SOURCE_ATTEMPTS {
        ...
        match validate_candidate(
            ...
            Err(message) if attempt + 1 == SOURCE_ATTEMPTS => {
                return Err(format!(
                    "agent generation for `{}` failed source audit after {SOURCE_ATTEMPTS} attempts: {message}",
                    item.callable.symbol
                ));
            }
            Err(message) => {
                ...
                feedback.push_str("Kotlin source audit failed:\n");
                feedback.push_str(&message);
                existing = Some(candidate.implementation);
            }
        }
    }
    unreachable!("Kotlin source attempt loop always returns")
}

fn combined_bindings(
    project: &Project,
    candidates: &BTreeMap<String, KotlinBinding>,
) -> Vec<KotlinBinding> {
...
fn effective_binding<'a>(
    project: &'a Project,
    candidates: &'a BTreeMap<String, KotlinBinding>,
    symbol: &str,
...
fn implicated_agent_symbols(message: &str, bindings: &[KotlinBinding]) -> BTreeSet<String> {
    let agents = bindings
        .iter()
        .filter(|binding| binding.owner == KotlinOwner::Agent)
        .collect::<Vec<_>>();
    let mut implicated = agents
        .iter()
        .filter(|binding| {
...
fn validate_complete(project: &Project, bindings: &[KotlinBinding]) -> Result<(), String> {
    let emission = emit::emit(&project.config, &project.plan, bindings)
        .map_err(|message| format!("emit complete Kotlin candidate: {message}"))?;
    if !emission.unresolved.is_empty() {
        return Err(format!(
            "complete Kotlin candidate unexpectedly has unresolved callables: {}",
            emission.unresolved.join(", ")
        ));
    }
    verify::verify(&project.config, &project.paths, &project.plan, &emission).map(|_| ())
}
...
impl AgentWorkspace {
    fn create() -> Result<Self, String> {
        loop {
            ...
            let root =
                std::env::temp_dir().join(format!("cott-kotlin-agent-{}-{id}", std::process::id()));
            match fs::create_dir(&root) {
                Ok(()) => {
                        ...
                        fs::create_dir(&workspace).and_then(|_| fs::create_dir(&scratch))
                    {
                        ...
                        return Err(format!("create Kotlin agent workspace: {error}"));
                    }
                    return Ok(Self {
                        root,
                        workspace,
                ...
                Err(error) => return Err(format!("create Kotlin agent workspace: {error}")),
            }
        }
    }
}
