...
const RUNTIME_TEMPLATE: &str = r####"// Cott's compiler-owned Kotlin/JVM runtime.
...
public object CottRuntime {
    public val PROJECT_NAME: String = __COTT_PROJECT_NAME_LITERAL__
    public val PROJECT_VERSION: String = __COTT_PROJECT_VERSION_LITERAL__
    public val COMPILER_VERSION: String = __COTT_RUNTIME_VERSION_LITERAL__
...
pub fn render_runtime(project_name: &str, project_version: &str) -> BTreeMap<PathBuf, Vec<u8>> {
    let source = substitute_runtime_markers(&[
        (
            ...
            kotlin_string_literal(project_name),
        ),
        (
            "__COTT_PROJECT_VERSION_LITERAL__",
            kotlin_string_literal(project_version),
        ),
        (
            "__COTT_RUNTIME_VERSION_LITERAL__",
            kotlin_string_literal(env!("CARGO_PKG_VERSION")),
        ),
    ]);
    BTreeMap::from([(
        PathBuf::from("kotlin/cott_runtime/CottRuntime.kt"),
        source.into_bytes(),
    )])
}
