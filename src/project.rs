...
pub fn discover_sources(
    ...
    if !project.root.is_absolute() || !project.source_dir.is_absolute() {
        return Err(ProjectError::InvalidProject {
            message: "project paths must be absolute",
        });
    }
    ensure_no_symlinks(&project.root)?;
    ensure_no_symlinks(&project.source_dir)?;
    let source_meta =
        fs::symlink_metadata(&project.source_dir).map_err(|source| ProjectError::Io {
            operation: "stat source directory",
    ...
    collect_sources(&project.source_dir, &project.source_dir, &mut files)?;
