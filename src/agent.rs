...
fn append_python_rules(
    ...
    if features.scenario {
        prompt.push_str("Scenario fixtures and steps are runner-owned. Scenario calls are facade-only: invoke the exact generated public facade, never a private `_cott_impl` implementation or `cott_bindings` module. The only private runtime effect adapters are `cott_runtime._cott_fixture_read`, `cott_runtime._cott_fixture_write`, `cott_runtime._cott_fixture_replace`, `cott_runtime._cott_fixture_http`, and `cott_runtime._cott_fixture_now`. They MAY be used only when the contract is targeted by a compatible declared scenario, and they act only inside that scenario's active compiler-owned fixture. Otherwise, an effectful callable MUST NOT invent an adapter name or authority. Do not emulate a fixture effect with stdlib I/O: never reach fixture files through standard-library file access. Do not inspect adapter internals, dynamically import an adapter, or retain an adapter value.\n");
